...
macro_rules! is_defs {
    ($($category:literal $status_type:ident => $min:literal..$max:literal),*,) => {
        impl Status {
            $(
                #[doc = concat!("Returns true if the status code is a ", $category, " (`", $min, "..", $max, "`).")]
                pub fn $status_type(&self) -> bool {
                    ($min..$max).contains(&self.code())
                }
            )*
        }
    };
...
is_defs! {
    "informational" is_informational => 100..200,
    "success"       is_success       => 200..300,
    "redirect"      is_redirect      => 300..400,
    "client error"  is_client_error  => 400..500,
    "server error"  is_server_error  => 500..600,
}
